...
class ZipFile implements Countable, ArrayAccess, Iterator {
	 ...
	 * @param mixed                $entry_name Entry name.
	 * @param array                $options File options.
	 * @return false|ZipEntry
	 * @throws Exception Throws Exception.
	 */
	public function add_spl_file( SplFileInfo|ZipEntry $file, mixed $entry_name = null, array $options = array() ): ZipEntry|bool {
		...
		if ( $file->isLink() ) {
			...
			} elseif ( $file_size < 512 ) {
				$compression_method = ZipCompressionMethod::STORED;
			} else {
				$is_bad_compression = FilesUtil::is_bad_compression_file( $file->getPathname() );
				if ( $is_bad_compression ) {
					$compression_method = ZipCompressionMethod::STORED;
				} else {
	 ...
	 * @return void
	 */
	protected function add_zip_entry( ZipEntry $zip_entry ): void {
	 ...
	 * @param mixed|null $entry_name Zip Entry name.
	 * @param mixed|null $compression_method Compression method. If null, then auto choosing method.
	 *
	 * @return ZipFile
	 * @throws Exception Throws Exception.
	 */
	public function add_file( string $filepath, mixed $entry_name = null, mixed $compression_method = null ): self {
		...
		$spl_file_info = new SplFileInfo( $filepath );
	 ...
	 * @throws Exception Throws Exception.
	 */
	public function add_from_string( string $entry_name, string $contents, mixed $compression_method = null ): self {
	 ...
	 * @param mixed|null $compression_method Compression method.
	 *
	 * @return ZipFile
	 * @throws Exception Throws Exception.
	 */
	public function add_files_from_iterator( Iterator $iterator, string $local_path = '/', mixed $compression_method = null ): ZipFile {
		if ( empty( $local_path ) ) {
			$local_path = '';
		}
	 ...
	 * @throws Exception Throws Exception.
	 */
	private function do_add_files( string $file_system_dir, array $files, string $zip_path, $compression_method = null ): void {
	 ...
	 */
	public function add_all( array $map_data ): void {
	 ...
	 * @throws Exception Throws Exception.
	 */
	protected function write_zip_to_stream( $handle ): void {
		$this->on_before_save();
