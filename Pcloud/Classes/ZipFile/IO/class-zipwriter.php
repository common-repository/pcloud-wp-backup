...
class ZipWriter {
	 ...
	 */
	public function write( $out_stream ): void {
	 ...
	 * @throws Exception Throws Exception.
	 */
	protected function write_local_header( $out_stream, ZipEntry $entry ): void {
		...
		foreach ( $collection as $extra_field ) {
			if ( $local ) {
				try {
					...
					$error_tag = '<span style="color: red">ERROR: </span>';

					wp2pclouddebugger::log( $error_tag . ' the following file can not be added to archive: ' . $entry->get_name() );

					throw new Exception( $error_tag . ' failed to prepare ZIP for entry: ' . $entry->get_name() );
				}
			} else {
	 ...
	 * @throws Exception Throws Exception.
	 */
	protected function write_data( $out_stream, ZipEntry $entry ): void {
	 ...
	 * @return void
	 */
	protected function write_data_descriptor( $out_stream, ZipEntry $entry ): void {
	 ...
	 * @throws Exception Throws Exception.
	 */
	protected function write_central_directory_block( $out_stream ): void {
	 ...
	 * @throws Exception Throws Exception.
	 */
	protected function write_central_directory_header( $out_stream, ZipEntry $entry ): void {
	 ...
	 */
	protected function write_end_of_central_directory_block( $out_stream, int $central_directory_offset, int $central_directory_size ): void {
