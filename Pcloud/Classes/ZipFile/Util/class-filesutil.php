...
final class FilesUtil {
	...
	public static function is_bad_compression_file( string $file ): bool {
		...
		$file_mime_by_extions = self::get_mime_type_by_extension( $file );
		if ( is_string( $file_mime_by_extions ) ) {
			return $file_mime_by_extions;
		}

		$handle = @fopen( $file, 'rb' ); // Open in binary read mode.
		if ( ! $handle ) {
			return false;
		}
		$data       = '';
		...
		while ( false !== $line && 1000 > $bytes_read ) {
			$data       .= $line;
			$bytes_read += strlen( $line );
		}
		@fclose( $handle );

		if ( 10 < $bytes_read && ! empty( $data ) ) {
			$mime_type = self::get_mime_type_from_string( $data );
		} else {
	 ...
	 * @return string|null
	 */
	private static function get_mime_type_by_extension( string $file_path ): ?string {
